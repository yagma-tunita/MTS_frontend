<template>
  <div class="vessel-manage-page">
    <el-card>
      <template #header><span>船舶管理</span><div style="float: right; display: flex; gap: 8px"><el-autocomplete v-model="query.keyword" :fetch-suggestions="searchSuggestions" placeholder="搜索船舶名称/IMO" clearable style="width: 220px" @select="onSearchSelect" @keyup.enter="handleSearch" /><el-button size="small" @click="handleSearch">搜索</el-button><el-button size="small" @click="handleExportVessels">导出</el-button><el-button size="small" @click="handleImportClick">导入</el-button><el-button size="small" type="primary" @click="openCreate">添加船舶</el-button></div><input type="file" ref="fileInputVessel" accept=".xlsx" style="display:none" @change="handleImportVessels" /></template>
      <el-table :data="list" border stripe v-loading="loading">
        ...
        <el-table-column label="状态" width="80"><template #default="{ row }"><el-tag :type="row.is_available === 1 ? 'success' : 'danger'" size="small">{{ row.is_available === 1 ? '可用' : '不可用' }}</el-tag></template></el-table-column>
...
const loading = ref(false); const saving = ref(false); const list = ref([]); const meta = ref({}); const companies = ref([]); const fileInputVessel = ref(null); const searchOptions = ref([])
...
async function loadSearchOptions() { try { const res = await getVesselListApi({ page: 1, page_size: 200 }); searchOptions.value = (res.data || []).map(v => ({ value: v.vessel_name })) } catch {} }
...
onMounted(async () => { await loadData(); try { const res = await getShippingCompanyListApi({ page: 1, page_size: 100 }); companies.value = res.data || [] } catch {}; loadSearchOptions() })
</script>
