<template>
  <div class="vessel-list-page"><el-card><template #header><span>船舶列表</span></template>
    <el-table :data="list" border stripe v-loading="loading">
      <el-table-column prop="vessel_id" label="ID" width="60" />
      <el-table-column prop="vessel_name" label="船舶名称" min-width="150" />
      <el-table-column prop="imo_number" label="IMO" width="130" />
      <el-table-column prop="max_deadweight_ton" label="载重(吨)" width="110" />
      <el-table-column prop="vessel_type" label="类型" width="100" />
      <el-table-column label="操作" width="100"><template #default="{ row }"><el-button size="small" @click="$router.push(`/shipper/vessel/detail/${row.vessel_id}`)">详情</el-button></template></el-table-column>
    </el-table>
