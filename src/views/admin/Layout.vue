  ...
  <div class="layout">
    <div class="sidebar">
      ...
      <el-menu :default-active="activeMenu" router background-color="#001529" text-color="rgba(255,255,255,0.65)" active-text-color="#fff">
        <el-menu-item index="/admin/dashboard"><el-icon><Odometer /></el-icon><span>工作台</span></el-menu-item>
        <el-menu-item index="/admin/shipper"><el-icon><UserFilled /></el-icon><span>货主管理</span></el-menu-item>
        <el-menu-item index="/admin/shipping"><el-icon><Van /></el-icon><span>海运公司管理</span></el-menu-item>
        <el-menu-item index="/admin/admin/list"><el-icon><Setting /></el-icon><span>管理员管理</span></el-menu-item>
        <el-menu-item index="/admin/city"><el-icon><OfficeBuilding /></el-icon><span>城市管理</span></el-menu-item>
        <el-menu-item index="/admin/port"><el-icon><MapLocation /></el-icon><span>港口管理</span></el-menu-item>
        <el-menu-item index="/admin/order/list"><el-icon><List /></el-icon><span>订单管理</span></el-menu-item>
        <el-menu-item index="/admin/vessel"><el-icon><Ship /></el-icon><span>船舶管理</span></el-menu-item>
        <el-menu-item index="/admin/line"><el-icon><Connection /></el-icon><span>航线管理</span></el-menu-item>
        <el-menu-item index="/admin/cargo"><el-icon><Box /></el-icon><span>货物管理</span></el-menu-item>
        <el-menu-item index="/admin/notification"><el-icon><Bell /></el-icon><span>发送通知</span></el-menu-item>
      </el-menu>
    ...
    <PasswordChangeDialog :visible="passwordDialogVisible" role="admin" :user-id="userIdStr" @close="passwordDialogVisible = false" />
  </div>
</template>
...
import { Ship, Odometer, List, UserFilled, Van, Setting, MapLocation, Connection, Box, OfficeBuilding, ArrowDown, Bell } from '@element-plus/icons-vue'
...
const pageTitle = computed(() => {
  const map = { '/admin/dashboard': '工作台', '/admin/order/list': '订单管理', '/admin/shipper': '货主管理', '/admin/shipping': '海运公司管理', '/admin/admin/list': '管理员管理', '/admin/city': '城市管理', '/admin/port': '港口管理', '/admin/vessel': '船舶管理', '/admin/line': '航线管理', '/admin/cargo': '货物管理', '/admin/notification': '发送通知' }
  for (const [k, v] of Object.entries(map)) { if (route.path.startsWith(k)) return v }; return ''
})
const activeMenu = computed(() => {
  const path = route.path
  if (path.startsWith('/admin/order/')) return '/admin/order/list'
  if (path.startsWith('/admin/port')) return '/admin/port'
  if (path.startsWith('/admin/vessel')) return '/admin/vessel'
  if (path.startsWith('/admin/line')) return '/admin/line'
  if (path.startsWith('/admin/cargo')) return '/admin/cargo'
  if (path.startsWith('/admin/city')) return '/admin/city'
  if (path.startsWith('/admin/admin')) return '/admin/admin/list'
  if (path.startsWith('/admin/shipper')) return '/admin/shipper'
  if (path.startsWith('/admin/shipping')) return '/admin/shipping'
  if (path.startsWith('/admin/notification')) return '/admin/notification'
  return path
})
