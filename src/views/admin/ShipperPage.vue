<template>
  <div class="shipper-page">
    <el-card>
      <template #header><span>货主管理</span><div style="float: right; display: flex; gap: 8px"><el-autocomplete v-model="query.keyword" :fetch-suggestions="searchSuggestions" placeholder="搜索公司名称/法人" clearable style="width: 220px" @select="onSearchSelect" @keyup.enter="handleSearch" /><el-button size="small" @click="handleSearch">搜索</el-button></div></template>
    ...
    <el-dialog v-model="editVisible" title="编辑货主" width="500px">
      <el-form :model="editForm" label-width="120px">
        ...
        <el-form-item label="法定代表人" prop="legal_representative"><el-input v-model="editForm.legal_representative" /></el-form-item>
...
import { getShipperListApi, updateShipperApi, deleteShipperApi } from '@/api/admin'
...
const editForm = reactive({ company_name: '', login_username: '', legal_representative: '', contact_phone: '', address: '', account_status: 1 })
let editId = null

async function loadData() { loading.value = true; try { const res = await getShipperListApi(query); list.value = res.data || []; meta.value = res.meta || {} } catch (e) { ElMessage.error(e.message || '加载失败') } finally { loading.value = false } }
function handleSearch() { query.page = 1; loadData() }
function openEdit(row) { editId = row.company_id; editForm.company_name = row.company_name; editForm.login_username = row.login_username; editForm.legal_representative = row.legal_representative; editForm.contact_phone = row.contact_phone; editForm.address = row.address; editForm.account_status = row.account_status; editVisible.value = true }
async function handleSave() { if (!editId) return; saving.value = true; try { await updateShipperApi(editId, { ...editForm }); ElMessage.success('更新成功'); editVisible.value = false; await loadData() } catch (e) { ElMessage.error(e.message || '更新失败') } finally { saving.value = false } }
async function handleDelete(row) { try { await ElMessageBox.confirm(`确认删除货主 "${row.company_name}"？`, '提示'); await deleteShipperApi(row.company_id); ElMessage.success('已删除'); await loadData() } catch { /* ignore */ } }
async function loadSearchOptions() { try { const res = await getShipperListApi({ page: 1, page_size: 200 }); searchOptions.value = (res.data || []).map(s => ({ value: s.company_name })) } catch {} }
